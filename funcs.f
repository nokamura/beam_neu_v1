...
C     Modified @U.Tokyo Dec.20 2014
C     ****************************************************
      implicitnone
      include 'inc/const.inc'
C     CONSTANTS
C     ARGUMENTS
      integer v1,v2,beam_sign,detect_sign,nn,sort,oct,idmatm_2
...
c         prob_approx2 = 1d0 -s2atm_2*(1d0 +Amu)*dsin( Del13/2d0 +Bmu )**2
      ...
      real*8 function prob_approx3(v1,v2,E,L,s212_2,s223_2,s213_2,dm21_2
     &     ,dm31_2,dCP,rho,oct)
C     ****************************************************
C     Approximated Oscillation Probability
C     eq.22 in 1209.2763
...
C     CONSTANTS
C     ARGUMENTS
      integer v1,v2,beam_sign,detect_sign,oct
      real*8 E,L,s212_2,s223_2,s213_2,dm21_2,dm31_2,dCP,rho
C     LOCAL VARIABLES
      integer i,j,k
      real*8 rho2,a
      real*8 s12_2,s23_2,s13_2,s212,s223,s213,s12,s23,s13
      real*8 c212_2,c223_2,c213_2,c12_2,c23_2,c13_2,c212,c223,c213
      real*8 c12,c23,c13,t23_2,t23,dCP2,Del12,Del13
      real*8 Amu,Bmu,Ae,Be,Ce
...
c      a = 2*dsqrt(2d0)*GF*E*ne
      a = 7.56d-5*rho2*E 
      Del12 = 2.534*dm21_2/(2*E)*L
      Del13 = 2.534*dm31_2/(2*E)*L
...
c      Ae = 0d0
      Ae = a*L/(2*Del13*E) -Del12*s212/(t23*s13)*dsin(dCP2)
c      Ae = a*L/(2*Del13*E) -Del12/3d0*s212/(t23*s13)*dsin(dCP2)

c      Be = 0d0
      Be = -a*L/(4*E) +Del12/2d0*s212/(t23*s13)*(dcos(dCP2) -2*s12_2)

c      Ce = 0d0
c      Ce = Del12**2*s212_2*c23_2 
      
      if ( (abs(v1).eq.2).and.(abs(v2).eq.1) ) then
         prob_approx3 = 4*s23_2*s13_2*( ( 1d0 +Ae )
     &        *dsin(Del13)**2 +Be*dsin(2*Del13) ) +Ce
c     &        *dsin(Del13)**2 +Be*2*dsin(Del13)**2 ) +Ce
      elseif ( (abs(v1).eq.2).and.(abs(v2).eq.2) ) then
         prob_approx3 = 1d0 -s223_2*((1d0 +Amu)*dsin(Del13)**2 
     &        +Bmu*dsin(2*Del13)) 
      else
         prob_approx3 = 0d0
      endif
